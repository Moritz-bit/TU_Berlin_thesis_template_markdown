...
\usepackage{chngcntr}
\usepackage{csquotes}
\usepackage{verbatim} % zum auskommentieren von Bloecken
\usepackage{xcolor} % zum einfaerben des Uni Logos
\usepackage{tabularx} % zum erstellen von Tabellen
\usepackage{ulem}	% unterstreichen im Text

% in order to get rid of error: "! LaTeX Error: Environment cslreferences undefined."
$if(csl-refs)$
...
\fancyhead[LO,RE]{\MakeUppercase{\slshape \leftmark}}
\fancyfoot[C]{\thepage}
...
% if urlcolor is set use italic font            
$if(urlcolor)$            
\makeatletter
    \def\url@itstyle{%
